---
name: predict
description: ローカルの dev サーバー経由で Jev にセトリを予想させ、結果を predictions/ に JSON 保存して要約する。Jev の呼び出しは有料なので、ユーザーが明示的に頼んだときだけ使う。
argument-hint: [--rumors "匂わせメモ"] [--size 24]
...
2. **実行**（1 回だけ。失敗しても自動でリトライしない）:
   ...
   出力先は既定で `predictions/<今日の日付>.json`。同日に複数回回すなら `--out` で名前を変える。
